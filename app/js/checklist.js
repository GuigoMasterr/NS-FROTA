...
function precisaCintasIcar(categoriaId) {
  return CATEGORIAS_CINTAS_ICAR.includes(categoriaId);
}
// ✅ Verifica se precisa de cintas de catraca e catracas
function precisaCatraca(categoriaId) {
  return CATEGORIAS_CATRACA.includes(categoriaId);
}
...
function abrirModalChecklist() {
  ...
  window.verificarQuantidade = function(input) {
    ...
    if (statusSpan) {
      statusSpan.innerHTML = qtd >= obrig
        ? '<span class="text-green-600">✅ OK</span>'
        : `<span class="text-red-600">❌ Faltam ${obrig - qtd}</span>`;
    }
    verificarTodosItens();
  };
  ...
  window.atualizarItensPorCategoria = function() {
    ...
    document.querySelectorAll('.cinta-item').forEach(el => {
      ...
      if (tipo === 'cinta-icarga') el.classList.toggle('hidden', !precisaIcar);
      else if (tipo === 'cinta-catraca' || tipo === 'catraca') el.classList.toggle('hidden', !precisaCat);
    });
...
function carregarTabelaChecklist(filtroPlaca = 'todos') {
  ...
  corpo.innerHTML = dados.length ? dados.map(c => {
    const statusClasse = c.statusGeral === 'OK' ? 'text-green-600' : 'text-amber-500';
    return `<tr>
